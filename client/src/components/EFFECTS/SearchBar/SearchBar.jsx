...
export const SearchBar = ({ className, recents, searchData }) => {
    ...
    const showData = searchWord
        ? data.filter((i) =>
            ...
            i.code.toLowerCase().includes(searchWord.toLowerCase())
        )
        : [];
                    ...
                    {showData.length > 0 ? (
                        <ul className="flex flex-col gap-y-2 max-h-60 max-w-80 overflow-y-auto p-2">
                            {showData.map((item, index) => (
                                <li key={index} className="cursor-pointer hover:bg-gray-100 p-2">
                                    {item.name} - {item.code}
                                </li>
                            ))}
                        </ul>
